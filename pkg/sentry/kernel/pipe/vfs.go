...
func (fd *VFSPipeFD) CopyInTo(ctx context.Context, ars usermem.AddrRangeSeq, dst safemem.Writer, opts usermem.IOOpts) (int64, error) {
	...
	n, err := fd.pipe.read(ctx, readOps{
		...
		read: func(view *buffer.View) (int64, error) {
			// TODO: This allows dst.WriteFromBlocks() to be called multiple
			// times, in violation of usermem.IO.CopyInTo()'s semantics. This
			// is unfortunate, but surprisingly appears to be consistent with
			// Linux's splice...
			n, err := view.ReadToWriter(safemem.ToIOWriter{dst}, count)
			view.TrimFront(n)
			return n, err
		},
	})
...
func (fd *VFSPipeFD) CopyOutFrom(ctx context.Context, ars usermem.AddrRangeSeq, src safemem.Reader, opts usermem.IOOpts) (int64, error) {
	...
	n, err := fd.pipe.write(ctx, writeOps{
		...
		write: func(view *buffer.View) (int64, error) {
			// TODO: see CopyInTo
			return view.WriteFromReader(safemem.ToIOReader{src}, count)
		},
	})
	if n > 0 {
		fd.pipe.Notify(waiter.EventIn)
		...
		write: func(dstView *buffer.View) (int64, error) {
			return src.pipe.readLocked(ctx, readOps{
				...
				read: func(srcView *buffer.View) (int64, error) {
					// We can't call dstView.WriteFromReader(srcView) because
					// it may make multiple calls to io.Reader.Read() =>
					// buffer.View.ReadToBlocks(), which does not "advance the
					// [buffer] read index" between calls.
					n, err := srcView.ReadToWriter(safemem.ToIOWriter{dstView}, count)
...
// spliceMu serializes attempts to lock Pipe.mu in multiple Pipes. It is needed
// to avoid circular lock ordering between calls to spliceOrTee.
