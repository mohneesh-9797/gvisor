...
// newSymlinkFD is like newFileFD, but for symlinks.
func newSymlinkFD(ctx context.Context, mode linux.FileMode) (*vfs.FileDescription, func(), error) {
	creds := auth.CredentialsFromContext(ctx)
	vfsObj, root, cleanup, err := newTmpfsRoot(ctx)
	if err != nil {
		return nil, nil, err
	...
	name := fmt.Sprintf("tmpfs-test-%d", atomic.AddInt64(&nextFileID, 1))
	if err := vfsObj.SymlinkAt(ctx, creds, &vfs.PathOperation{
		Root:  root,
		Start: root,
		Path:  fspath.Parse(name),
	}, "target"); err != nil {
		return nil, nil, fmt.Errorf("SymlinkAt failed: %v", err)
	}
	fd, err := vfsObj.OpenAt(ctx, creds, &vfs.PathOperation{
		Root:  root,
		Start: root,
	...
	if err != nil {
		cleanup()
		return nil, nil, fmt.Errorf("failed to open symlink %q: %v", name, err)
	}
