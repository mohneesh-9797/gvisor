...
// an anoymouys fd created by memfd_create.
func NewMemfd(mount *vfs.Mount, creds *auth.Credentials, allowSeals bool, name string) (*vfs.FileDescription, error) {
	...
	if !ok {
		panic("NewMemfd() called with non-tmpfs moun")
	}
	rootDentry, ok := mount.Root().Impl().(*dentry)
	if !ok {
		panic("NewMemfd() called with non-tmpfs root")
	}
	rootDir := rootDentry.inode.impl.(*directory)
	if !ok {
		panic("NewMemfd() called with non-directory root")
	}
	...
	defer d.DecRef()
	fs.mu.Lock()
	rootDir.insertChildLocked(d, name)
	fs.mu.Unlock()
