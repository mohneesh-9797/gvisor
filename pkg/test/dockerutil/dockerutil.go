...
// ListNetworks returns the network interfaces of the container.
func (d *Docker) ListNetworks() (map[string]NetworkInterface, error) {
	const format = `{{json .NetworkSettings.Networks}}`
	out, err := testutil.Command(d.logger, "docker", "inspect", "-f", format, d.Name).CombinedOutput()
	if err != nil {
		return nil, fmt.Errorf("error network interfaces: %v", err)
	}

	networks := map[string]map[string]string{}
	if err := json.Unmarshal(out, &networks); err != nil {
		return nil, fmt.Errorf("error decoding network interfaces: %v", err)
	}
	...
	for name, iface := range networks {
		...
		if rawMac != "" {
			...
			if err != nil {
				return nil, fmt.Errorf("invalid MAC: %q: %v", rawMac, err)
			}
			netface.MAC = mac
		}
