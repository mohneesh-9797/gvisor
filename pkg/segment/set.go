...
// trackGaps must be either 0 or 1.
const trackGaps = 0
...
// Value returns the value of the gap.
//
// Precondition: trackGaps must be non-zero.
func (d *dynamicGap) Value() Key {
	return d[:][0]
}
...
func (s *Set) Insert(gap GapIterator, r Range, val Value) Iterator {
	...
	if prev.Ok() && prev.End() == r.Start {
		if mval, ok := (Functions{}).Merge(prev.Range(), prev.Value(), r, val); ok {
			shrinkMaxGap := trackGaps != 0 && gap.Range().Length() == gap.node.maxGap.Value()
...
func (s *Set) InsertWithoutMergingUnchecked(gap GapIterator, r Range, val Value) Iterator {
	...
	splitMaxGap := trackGaps != 0 && (gap.node.nrSegments == 0 || gap.Range().Length() == gap.node.maxGap.Value())
...
func (n *node) updateMaxGapLeaf() {
	...
	for p := n.parent; p != nil; p = p.parent {
		...
		if p.maxGap.Value() == parentNewMax {
...
func (n *node) calculateMaxGapInternal() Key {
	max := n.children[0].maxGap.Value()
	for i := 1; i <= n.nrSegments; i++ {
		if current := n.children[i].maxGap.Value(); current > max {
			max = current
		}
	}
	return max
...
func (gap GapIterator) nextLargeEnoughGapHelper(minSize Key) GapIterator {
	...
	for gap.node != nil &&
		(gap.node.maxGap.Value() < minSize || (!gap.node.hasChildren && gap.index == gap.node.nrSegments)) {
...
func (gap GapIterator) prevLargeEnoughGapHelper(minSize Key) GapIterator {
	...
	for gap.node != nil &&
		(gap.node.maxGap.Value() < minSize || (!gap.node.hasChildren && gap.index == 0)) {
		gap.node, gap.index = gap.node.parent, gap.node.parentIndex
	}
	// If no large enough gap throughout the whole set, return a terminal gap iterator.
	if gap.node == nil {
...
// Check returns an error if s is incorrectly sorted, does not contain exactly
// expectedSegments segments, or contains a segment which fails the passed
// check.
//
// This should be used only for testing, and has been added to this package for
// templating convenience.
func (s *Set) Check(expectedSegments int, segFunc func(int, Range, Value) error) error {
	havePrev := false
	prev := Key(0)
	nrSegments := 0
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
...
// Count counts the number of segments in the set.
//
// Similar to Check, this should only be used for testing.
func (s *Set) Count() (segments int) {
	for seg := s.FirstSegment(); seg.Ok(); seg = seg.NextSegment() {
		segments++
	}
	return segments
