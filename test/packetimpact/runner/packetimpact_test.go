...
func TestOne(t *testing.T) {
	...
	runOpts := dockerutil.RunOpts{Image: "packetimpact", CapAdd: []string{"NET_ADMIN"}, Extra: []string{"--sysctl", "net.ipv6.conf.all.disable_ipv6=0", "--rm"}, Foreground: true, Pty: func(_ *exec.Cmd, _ *os.File) {}}
	...
	// needed.
	if remoteIPv6 == nil {
		dut.Exec(dockerutil.RunOpts{}, "ip", "addr", "add", netdevs.MACToIP(remoteMAC).String(), "scope", "link", "dev", dutTestDevice)
...
func addNetworks(d *dockerutil.Docker, addr net.IP, networks []*dockerutil.DockerNetwork) error {
	for _, dn := range networks {
		...
		if err := dn.Connect(d, "--ip", ip.String()); err != nil {
			return fmt.Errorf("unable to connect container %s to network %v: %s", d.Name, dn, err)
		}
	}
	return nil
}
...
func deviceByIP(d *dockerutil.Docker, ip net.IP) (string, netdevs.DeviceInfo, error) {
	...
	if err != nil {
		return "", netdevs.DeviceInfo{}, fmt.Errorf("listing devices on %s container: %v", d.Name, err)
	}
	devs, err := netdevs.ParseDevices(out)
	if err != nil {
		return "", netdevs.DeviceInfo{}, fmt.Errorf("parsing devices from %s container: %v", d.Name, err)
	}
	testDevice, deviceInfo, err := netdevs.FindDeviceByIP(ip, devs)
	if err != nil {
		return "", netdevs.DeviceInfo{}, fmt.Errorf("can't find deviceInfo for container %s: %s", d.Name, err)
	}
